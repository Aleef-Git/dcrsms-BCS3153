                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="overflow-x-auto">
                        <x-auth-validation-errors class="mb-4" :errors="$errors" />
                                    ...
                                    <tbody>
                                        @foreach ($service_request->quotations as $quotation)
                                            <tr>
                                                ...
                                                <td>
                                                    @if (($service_request->customer_approval == 'waiting' || $service_request->customer_approval == null) && Auth::user()->hasRole('staff'))
                                                        <form action="{{ route('quotation.delete', ['quotation'=>$quotation]) }}" onsubmit="return submitForm(this);" method="POST">
                                                            @method('DELETE')
                                                            @csrf
                            ...
                            @if (Auth::user()->hasRole('staff') && ($service_request->customer_approval == 'waiting' || $service_request->customer_approval == null))
                                <a class="btn mt-3" href="{{ route('quotation.create', ['service_request'=>$service_request]) }}">Add Quotation</a>
                            @elseif (Auth::user()->hasRole('customer') && $service_request->customer_approval == 'waiting')
